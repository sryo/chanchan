...
Quién, después `toca`, después qué: la bata, el bajo, la melodía, las cuerdas.
...
`-` es un silencio. `_` estira el paso anterior. `|` separa compases, y cada compás dura una vuelta con sus propios pasos. `!` pegado al paso lo acentúa.
...
`tres en ocho` pone tres golpes repartidos parejo en ocho pasos. También `cinco en ocho`, `tres en cuatro`, `cinco en dieciséis`, `siete en dieciséis` y `dos en tres`.

    la bata toca pum, tres en ocho

**De a ratos**

`cada dos vueltas al doble`, y lo mismo con tres, cuatro u ocho vueltas. O sin contar:

    de vez en cuando   una de cada cuatro vueltas, más o menos
    a veces            la mitad de las veces
    casi siempre       casi todas
    casi nunca         muy de tanto en tanto
...
`una vuelta sí y una no`, `dos vueltas sí y dos no`, `cuatro vueltas sí y cuatro no`, `ocho vueltas sí y ocho no`. Y desparejo: `tres vueltas sí y una no`, `una vuelta sí y tres no`, `dos vueltas sí y seis no`, `seis vueltas sí y dos no`.
...
Un nombre de una palabra y dos puntos. Lo que sigue es de ella. Sin `dura`, la sección dura lo que tardan sus renglones en volver a caer juntos.
...
Tiempos por minuto, de 20 a 400. Con `en tres` la vuelta tiene tres tiempos en vez de cuatro, y `va a 150 en tres` es un vals. Sirve también en dos, en seis y hasta doce.
...
Un renglón que empieza con `*` es para quien lee. No suena y no dice nada del tema. Si lleva un `@`, lo que sigue es un enlace a otro tema.

**Otro documento**

    @la base

Un renglón que empieza con `@` no suena: apunta a otro tema tuyo. Tipeá el `@` y te ofrece los que tenés. Para ir, apretá el nombre. Si ese tema no existe todavía, lo crea.
...
Se escriben con letras hasta dieciséis, o con cifras. Los arreglos y el `cada` llegan hasta 32 vueltas. Una sección, hasta 256.
