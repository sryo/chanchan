...
function armarVacio() {
  ...
  const caja = cajaVacio.querySelector('.ejemplos');
...
function abrirTemas() {
  panelTemas.innerHTML = '<h3>temas</h3>' + EJEMPLOS.map((e, i) =>
    '<div class="op" data-i="' + i + '">' + esc(e.nombre) +
    '<span class="d">' + /va a (\d+)/.exec(e.txt)[1] + '</span></div>').join('');
  panelTemas.querySelectorAll('.op').forEach(el => el.addEventListener('mousedown', ev => {
    ev.preventDefault();
    cargarTema(EJEMPLOS[+el.dataset.i]);
